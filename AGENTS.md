...
- `MarkupsScreenshotTool` (dispatched through `AsyncStartTool`, polled with `AsyncResultTool`) works but is too small to verify anything with. Tested September 8, 2026 from the National Rail Departures key: it accepts no arguments at all (`size`, `markup_size`, `layout`, `width`, `scale`, `device`, `model`, `format`, `thumbnail` are each rejected with "unknown keyword"), takes about five seconds, and returns a single 80x48 PNG of the key's plugin labelled "Screenshot of markup_full". Two runs returned identical bytes. There is no way to ask for another size, a device, or full resolution, and none of the layout or overflow analysis the in-app tool description promises comes back. Use the Visual Editor and the static-fixture method below instead.
