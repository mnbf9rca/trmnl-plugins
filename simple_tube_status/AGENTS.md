...
- The reason toggle does nothing on a busy day: with eight disrupted lines (the TRMNL reviewer's test day, September 2026) every layout's `max_with_reason_lg` is exceeded on both devices, so no reason line renders anywhere. That is by design (a reason row is 106px on the X full layout, so eight of them need 926px in a 711px board), and the toggle's help text says so. No X cap can reach eight, so the X caps were left alone.
...
- To measure real widths and heights, open the markup editor for one size (`/plugin_settings/<id>/markup/edit?size=markup_quadrant`) in Playwright and read `document.querySelector('iframe.rounded-2xl').contentDocument`: it is same-origin, so `getBoundingClientRect()` on `.column`, `table`, `tbody tr` and the spans gives CSS pixels (the X renders at 1040x780 CSS pixels, the original at 800x480). Switch device with the toolbar `<select>` whose options include "TRMNL X" (set its value and dispatch a `change` event), and wait for `document.fonts.status` to be `loaded` before measuring: the original device's bitmap font (`TRMNL12`) is wider than the fallback. Candidate classes can be swapped on the spans in that frame and re-measured before writing any markup. The settings page's Force Refresh link opens an in-page "Are you sure?" modal whose Confirm button must be clicked; `window.confirm` is not used.
